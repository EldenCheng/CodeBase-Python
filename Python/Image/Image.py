...
def get_average_hls(target_image: [str, Path], return_result='all'):
    ...
    """
    '''
    色相(h), 指的是颜色的纯度, 其中, 红色色相值一般为0(360), 黄色色相值约为为60, 绿色色相值约为120, 蓝色色相值约为180, 蓝紫色约为240, 紫红色约为300
             黑白灰没有色相值(只有饱和度与明度), 平均值越接近某一个原色的值, 表明图片整体上更加接近某一个原色
    饱和度(s), 指的是颜色的鲜艳程度, 其中, 平均值越小, 表明图像整体上更加接近中性灰色
    明度(l), 指的是颜色的明亮程度, 其中, 平均值越大, 图像整体颜色越浅越接近白色, 平均值越小, 图像整体颜色越深越接近黑色
    '''
