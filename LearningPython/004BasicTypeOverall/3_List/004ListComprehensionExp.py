...
# 方便地从每个元素中获取特定位置的元素
col =  [row[1] for row in l] # 这个表达式的意义是：遍历二维列表，从每一个一级元素中获得二级的第二个元素，然后组成一个新的列表
print("方便地从每个元素中获取特定位置的元素:", col)

# 还可以对获取到的元素进行一定运算或者使用条件语句判断进行一定筛选
col = [row[1] + 1 for row in l] # 对获取到的元素+1
...
# 按顺序汇总各项
total = [sum(row) for row in l]
print("按顺序汇总各项:", total)

# 使用解析表达式从列表生成字典
d = {i: sum(l[i]) for i in range(3)}
