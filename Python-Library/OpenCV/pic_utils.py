...
class CompressImage:
    ...
    @staticmethod
    def compress(pic_path: str, target_path: str,  method: str = 'm', size: float = 0.5, quality: int = 50):
        """
        ...
        try:
            ...
            if method:  # 设置是否需要额外压缩
                if method == "a": # 使用google Jpegli的第三方封装包来压缩, 压缩速度快比guetzli快得多,甚至比mozjpeg都快, 体积比mozjpeg小, 但同质量画质不如mozjpeg, 更远不如guetzli
                    # image = ajpegli.imread("input.jpg")
                    optimized_jpeg = ajpegli.encode(ajpegli.imread(target_path), quality=quality, progressive=0)
                else:
